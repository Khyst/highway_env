...
class ProjectIntersectionEnv(AbstractEnv):
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        self._spawn_vehicle(spawn_probability=self.config["spawn_probability"], go_straight=True)
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        for t in range(n_vehicles - 1):
            self._spawn_vehicle(np.linspace(0, 80, n_vehicles)[t], go_straight=True)
    ...
    def _spawn_vehicle(self,
        ...
        route = self.np_random.choice([1, 2, 3], size=2, replace=False)
