...
class MotionPlannerParkingEnv(AbstractEnv, GoalEnv):
    """
    ...
    def _info(self, obs, action) -> dict:
        info = super(MotionPlannerParkingEnv, self)._info(obs, action)
        if isinstance(self.observation_type, MultiAgentObservation):
            success = tuple(
                self._is_success(agent_obs['achieved_goal'], agent_obs['desired_goal']) for agent_obs in obs)
        else:
    ...
    def _create_vehicles(self) -> None:
        ...
        if self.config["parallel_scenario"]:
            vehicle = self.action_type.vehicle_class(
                self.road, self.goal.position + np.array([-4, -20]), lane.heading, 0
            )
        else:
            vehicle = self.action_type.vehicle_class(
                self.road, self.goal.position + np.array([-10, -9]), 0, 0
            )
    ...
    def _reward(self, action: np.ndarray) -> float:
        ...
        reward = sum(
            self.compute_reward(agent_obs['achieved_goal'], agent_obs['desired_goal'], {}) for agent_obs in obs)
